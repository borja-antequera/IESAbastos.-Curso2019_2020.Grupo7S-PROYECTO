    ...
    <div class="col-12">
      <div class="single-chat-tab">
            ...
            <div class="chat-body">
                ...
                @foreach($mensajes_director_tutor as $mensaje_director_tutor)
                    <!-- Si el emisor del mensaje es el usuario logueado, aplicamos la clase sender (rosa y a la derecha)-->
                    @if($mensaje_director_tutor->emisor_id == $emisor_id)
                        <div class="message-content sender">
                            <label for=""><?php echo date("d/m/Y H:i", strtotime($mensaje_director_tutor->created_at)) ?></label>
                            <div class="msg-block">
                            <p>
                                {{ $mensaje_director_tutor->mensaje }}
                            </p>
                            </div>
                        </div>
                    @else
                        ...
                        <div class="message-content receiver">
                            <label for=""><?php echo date("d/m/Y H:i", strtotime($mensaje_director_tutor->created_at)) ?></label>
                            <div style="background-color:{{ agendaInfantil\TipoMensaje::find($mensaje_director_tutor->tipo_mensaje_id)->color}};" class="msg-block">
                            <p>
                                <i class="fa fas {{ agendaInfantil\TipoMensaje::find($mensaje_director_tutor->tipo_mensaje_id)->icono}}"></i> {{ $mensaje_director_tutor->mensaje }}
                            </p>
                            </div>
                        </div>
